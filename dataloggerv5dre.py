...
if not Path(nnBlobPath).exists():
    import sys
    raise FileNotFoundError(f'Blob file not found')
...
# Defining functions
def append_data_to_workbook(sheet, data):
    """Append the data to the workbook."""
    sheet.append(data)
    book.save('detections.xlsx')
    
def log_image_data():
    """Log the image data to the workbook if logging is enabled."""
    global last_append_time, logging_enabled
    detections = depthai.get_detections()
...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        for detection in detections:
            ...
            data_to_log = labelMap[detection.label], time_ref, detection.confidence * 100, detection.spatialCoordinates.z / 1000
